...
module PodSpecModifier
    ...
    def self.restore_kit_path_in_podspec(showLog = false)
        ...
        @modifys.each do |podspec_path, data|
            ...
            result = restore(podspec_path, data[:original_value])
            if result[:success]
    ...
    def self.apply_all_modifys_for(podspec_path)
        ...
        # 读取并备份文件内容
        begin
            data[:backup_content] = File.read(podspec_path)
            rescue => e
    ...
    (['"])           # 捕获引号类型（单或双）
     ([^'"]*)         # 捕获路径值
      ...
      unless content.match(KIT_PATH_REGEX)
      ...
      # 恢复podspec文件中的kit_path值
      def self.restore(podspec_path, original_value)
